...
const makeMessagesSocket = (config) => {
    ...
    const assertSessions = async (jids, force) => {
        ...
        if (force) {
            ...
            // Group jids by user for batch processing
            ...
            // Pre-check LID mappings in parallel for all users
            const userMappingChecks = await Promise.all(
                Array.from(userGroups.entries()).map(async ([user, userJids]) => {
                    const hasUserJid = userJids.some(jid => WABinary_1.isJidUser(jid))
                    if (!hasUserJid) {
                        return { user, shouldMigrate: false, lidForPN: undefined }
                    }
                    ...
                    try {
                        ...
                        if (mapping?.includes('@lid')) {
                            logger.debug({ user, lidForPN: mapping }, 'User has LID mapping')
                            return { user, shouldMigrate: true, lidForPN: mapping }
                        }
                    } catch (error) {
                        logger.debug({ user, error }, 'Failed to check LID mapping')
                    }
                    return { user, shouldMigrate: false, lidForPN: undefined }
                })
            )

            const mappingResults = new Map(userMappingChecks.map(result => [result.user, result]))

            // Process migrations and build fetch list
            for (const [user, userJids] of userGroups) {
                const mappingResult = mappingResults.get(user)
                ...
                if (shouldMigrateUser && lidForPN) {
                    try {
                        ...
                        if (migrationResult.migrated > 0) {
                            ...
                            }, 'Completed bulk migration')
                        }
                    } catch (error) {
                        logger.warn({ user, error }, 'Migration failed')
                    }
                }

                // Add missing sessions to fetch list
    ...
    const createParticipantNodes = async (jids, message, extraAttrs, dsmMessage) => {
        ...
        // Pre-fetch LID mappings for all users in parallel
        const userMappings = new Map()
        const mappingPromises = Array.from(devicesByUser.keys()).map(async (user) => {
            try {
                const pnJid = `${user}@s.whatsapp.net`
                const lidMapping = await signalRepository.lidMapping.getLIDForPN(pnJid)
                if (lidMapping?.includes('@lid')) {
                    userMappings.set(user, lidMapping)
                }
            } catch (error) {
                logger.error(`Silent fail - we'll use original jid: ${user}@s.whatsapp.net`)
            }
        })
        ...
        const userEncryptionPromises = Array.from(devicesByUser.entries()).map(([user, userDevices]) => 
            encryptionMutex.mutex(user, async () => {
                ...
                for (const { recipientJid: wireJid, patchedMessage } of userDevices) {
                    ...
                    // DSM message handling - simplified logic
                    if (dsmMessage) {
                        const targetUser = WABinary_1.jidDecode(wireJid)?.user
                        const ownPnUser = WABinary_1.jidDecode(meId)?.user
                        const isOwnUser = targetUser === ownPnUser
                        const isExactSenderDevice = wireJid === meId || wireJid === meLid
                    ...
                    // Determine encryption JID - use cached mapping
                    let encryptionJid = wireJid
                    if (WABinary_1.isJidUser(wireJid)) {
                        const lidMapping = userMappings.get(user)
                        if (lidMapping) {
                            const wireDecoded = WABinary_1.jidDecode(wireJid)
                            const deviceId = wireDecoded?.device || 0
                            const lidDecoded = WABinary_1.jidDecode(lidMapping)
                            encryptionJid = WABinary_1.jidEncode(lidDecoded?.user, 'lid', deviceId)
                        }
                    }
