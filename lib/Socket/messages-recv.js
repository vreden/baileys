...
const makeMessagesRecvSocket = (config) => {
    ...
    global.groupMetadataCache = async (jid) => {
        ...
        if (group_cache[jid] && (timestamp - group_cache[jid].timestamp < duration)) return group_cache[jid].data
        const data = await groupMetadata(jid)
        group_cache[jid] = {
            data: data,
            timestamp: timestamp
        };
        const lid_mapping = data.participants.map(item => ({
            pn: item.id,
            lid: item.lid
        }));
        await signalRepository.lidMapping.storeLIDPNMappings(lid_mapping)
        return data
    }
