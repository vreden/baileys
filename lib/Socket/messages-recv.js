...
export const makeMessagesRecvSocket = (config) => {
    ...
    const { ev, authState, ws, processingMutex, signalRepository, query, upsertMessage, resyncAppState, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, groupMetadata, uploadPreKeys, sendPeerDataOperationMessage, messageRetryManager } = sock;
    global.groupMetadataCache = new Map()
    global.signalRepository = signalRepository
    ...
    const handleMessage = async (node) => {
        ...
        const alt = msg.key.participant_lid;
        if (!!alt) {
                    ...
                    if (msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        if (isJidGroup(msg.key.remoteJid)) {
                            ...
                            const getMetadata = async () => {
                                ...
                                if (!global.groupMetadataCache.has(groupJid)) {
                                    global.groupMetadataCache.set(groupJid, groupMetadata(groupJid))
                                }
                                return await global.groupMetadataCache.get(groupJid)
                            }
                            if (typeof participant === "string" && participant.endsWith('@lid')) {
                                if (msg.message?.[mtype]?.contextInfo) {
                                    ...
                                    if (found) {
                                        ...
                                        const lid = jidNormalizedUser(sender_lid)
                                        const sender_jid = await signalRepository.lidMapping.getPNForLID(sender_lid)
                                        if (sender_jid) {
                                            const pn = jidNormalizedUser(sender_jid)
                                            await signalRepository.lidMapping.storeLIDPNMappings([{ lid, pn }])
                                    ...
                                    for (const mentionedLid of mentionedJid) {
                                        if (typeof mentionedLid === "string" && mentionedLid.endsWith('@lid')) {
                                            ...
                                            if (found) {
                                                ...
                                                const lid = jidNormalizedUser(mentionedLid)
                                                const mentioned_jid = await signalRepository.lidMapping.getPNForLID(mentionedLid)
                                                if (mentioned_jid) {
                                                    const pn = jidNormalizedUser(mentioned_jid)
                                                    await signalRepository.lidMapping.storeLIDPNMappings([{ lid, pn }])
                        ...
                        if (category === 'peer') {
                            ...
                            type = 'sender';
                            if (isLidUser(msg.key.remoteJid) || isLidUser(msg.key.participant_lid)) {
                                participant = author;
                            }
                        }
