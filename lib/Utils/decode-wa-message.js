...
const serializeMessage = async (msg, repository) => {
    ...
    if (typeof participant === "string" && participant.endsWith('@lid')) {
        if (msg?.[mtype]?.contextInfo) {
            const sender_lid = msg[mtype].contextInfo.participant
            const sender_jid = await repository.lidMapping.getPNForLID(sender_lid)
            if (sender_jid) {
                const pn = WABinary_1.jidNormalizedUser(sender_jid)
                msg[mtype].contextInfo.participant = pn
            }
        }
    }
    ...
    if (Array.isArray(mentionedJid) && mentionedJid.length > 0) {
        if (msg?.[mtype]?.contextInfo) {
            const mentioned = []
            for (const mentioned_lid of mentionedJid) {
                if (typeof mentioned_lid === "string" && mentioned_lid.endsWith('@lid')) {
                    const mentioned_jid = await repository.lidMapping.getPNForLID(mentioned_lid)
                    if (mentioned_jid) {
                        const pn = WABinary_1.jidNormalizedUser(mentioned_jid)
                        mentioned.push(pn)
                        if (msg[mtype]?.text) {
                            msg[mtype].text = msg[mtype].text.replace(new RegExp(`@${mentioned_lid.split("@")[0]}`, "g"), `@${pn.split("@")[0]}`);
                        } else if (msg[mtype]?.caption) {
                            msg[mtype].caption = msg[mtype].caption.replace(new RegExp(`@${mentioned_lid.split("@")[0]}`, "g"), `@${pn.split("@")[0]}`);
                        }
                    } else {
                        mentioned.push(mentioned_lid)
                    }
                } else {
                    mentioned.push(mentioned_lid)
                }
            }
            msg[mtype].contextInfo.mentionedJid = mentioned
        }
    }
